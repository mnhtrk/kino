...
void PrintPodr(struct film l){
    ...
    printf("┃\n");
    printf("┃Страна: %s", l.country);
    for (int i = 0; i <= (41 - a2); i++){
        printf(" ");
    }
    printf("┃\n");
    printf("┃Жанр: %s", l.genre);
    for (int i = 0; i <= (43 - a3); i++){
        printf(" ");
    }
    printf("┃\n");
    printf("┃Рейтинг: %s", l.rating);
    for (int i = 0; i <= 37; i++){
        printf(" ");
    }
    printf("┃\n");
...
void PrintCat(struct list l, int index){
    ...
    printf("┃ %s", current->prev->film.name);
    printf("  ┃");
    printElem(3, ' ');
    printf("║ %s", current->film.name);
    printf("  ║");
    printElem(3, ' ');
    printf("┃ %s", current->next->film.name);
    printf(" ┃\n");
...
void PrintLich(struct user l){
    printf("Склонитесь смертные, %s вернулся!\n", l.login);
    printf("Логин: %s\n", l.login);
    printf("Пароль: %s\n", l.pass);
    printf("Номер карты: %s\n", l.card);
    printf("Избранных фильмов: %d\n", l.favSize);
    if (l.isAdmin == 1){
        printf("Статус админа: есть\n");
    } else {
        printf("Статус админа: нет\n");
    }
    printf("Что будете созерцать милорд?\n");
}
